...
# install.packages("tidyverse")  # optional, for data wrangling
library(tidyverse)

# Step 0: Clean and prepare the data
nested_boxes$occupied <- ifelse(nested_boxes$box.occupant == "empty" | is.na(nested_boxes$box.occupant), 0, 1)

# Step 1: Null model
mod1 <- glm(occupied ~ 1, family = binomial, data = nested_boxes)

# Step 2: Saturated model (interaction)
mod2 <- glm(occupied ~ season * edge.distance, family = binomial, data = nested_boxes)

# Step 3: Additive model
mod3 <- glm(occupied ~ edge.distance, family = binomial, data = nested_boxes)

# Step 4: Season only model
mod4 <- glm(occupied ~ season, family = binomial, data = nested_boxes)
...
```{r}
plot(simulateResiduals(mod1), main = "Model 1: Null model")
plot(simulateResiduals(mod2), main = "Model 2: Saturated model")
plot(simulateResiduals(mod3), main = "Model 3: Distance to edge only")
plot(simulateResiduals(mod4), main = "Model 4: Season only model")
```
